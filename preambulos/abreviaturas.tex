...
\begin{multicols}{2}
\begin{acronym}[SDS-PAGE] %%%%reemplazr por abrev más larga
%    \setlength{\itemsep}{8pt}
    \acro{acn}[ACN]{acetonitrilo}
    \acro{afm}[AFM]{microscopía de fuerza atómica}
    \acro{apl}[APL]{área por lípido}
    \acro{atr}[ATR]{reflectancia total atenuada}
    \acro{bam}[BAM]{microscopía de ángulo de Brewster}
    \acro{cg}[CG]{grano grueso}
    ...
    \acro{dm}[MD]{dinámica molecular}
    \acro{do}[DO$_{600}$]{densidad óptica a 600 nm}
    \acro{dppc}[DPPC]{dipalmitoilfosfatidilcolina}
    \acro{dtt}[DTT]{DL-Ditiotreitol}
    \acro{ec}[EC]{extracelular}
    \acro{ecoli}[\textit{E. coli}]{Escherichia coli}
    \acro{ecm}[ECM]{matrix extracelular}
    ...
    \acro{guvs}[GUVs]{vesículas unilamelares gigantes}
    \acro{hplc}[HPLC]{cromatografía líquida de alta presión}
    \acro{ic}[IC]{intracelular}
    \acro{imc}[IMC]{cierre membrana interior}
    \acro{iptg}[IPTG]{isopropil $\beta$-D-1-tiogalactopiranósido}
    \acro{itc}[ITC]{calorimetría de titulación isotérmica}
    \acro{lb}[LB]{lysogeny broth}
    \acro{lc}[LC]{líquido condensado}
    \acro{le}[LE]{líquido expandido}
    \acro{luvs}[LUVs]{vesículas unilamelares grandes}
    \acro{mbp}[MPB]{proteína de unión a maltosa}
    \acro{meoh}[MeOH]{metanol}
    \acro{mlvs}[MLVs]{vesículas multilamelares grandes}
    \acro{mm}[MM]{mecánica molecular}
    \acro{omc}[OMC]{cierre membrana exterior}
    \acro{pbs}[PBS]{buffer fosfato-salino}
    \acro{pdb}[PDB]{Protein Data Bank}
    \acro{pdb}[PDB]{banco de datos de proteínas}
    \acro{pes}[PES]{superficie de energía potencial}
    ...
    \acro{rdf}[RDF]{función de distribución radial}
    ...
    \acro{tri}[Trix]{tris (hidroximetil) aminometano}
